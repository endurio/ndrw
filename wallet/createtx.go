...
func makeInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	currentTotal := chainutil.Amount(0)
	currentInputs := make([]*wire.TxIn, 0, len(eligible))
	currentScripts := make([][]byte, 0, len(eligible))
	currentInputValues := make([]chainutil.Amount, 0, len(eligible))

	return func(target chainutil.Amount) (chainutil.Amount, []*wire.TxIn,
		[]chainutil.Amount, [][]byte, error) {
...
func (s secretSource) GetKey(addr chainutil.Address) (*chainec.PrivateKey, bool, error) {
...
func (s secretSource) GetScript(addr chainutil.Address) ([]byte, error) {
	ma, err := s.Address(s.addrmgrNs, addr)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, account uint32,
	...
	err = walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		...
		changeSource := func() ([]byte, error) {
			...
			var changeAddr chainutil.Address
			var err error
	...
	if tx.ChangeIndex >= 0 && account == waddrmgr.ImportedAddrAccount {
		changeAmount := chainutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
...
func validateMsgTx(tx *wire.MsgTx, prevScripts [][]byte, inputValues []chainutil.Amount) error {
