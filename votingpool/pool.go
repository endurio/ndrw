...
func (p *Pool) DepositScriptAddress(seriesID uint32, branch Branch, index Index) (chainutil.Address, error) {
	script, err := p.DepositScript(seriesID, branch, index)
	if err != nil {
		return nil, err
	}
...
func (p *Pool) addressFor(script []byte) (chainutil.Address, error) {
	scriptHash := chainutil.Hash160(script)
	return chainutil.NewAddressScriptHashFromHash(scriptHash, p.manager.ChainParams())
}
...
func (p *Pool) DepositScript(seriesID uint32, branch Branch, index Index) ([]byte, error) {
	...
	pks := make([]*chainutil.AddressPubKey, len(pubKeys))
	for i, key := range pubKeys {
		...
		pks[i], err = chainutil.NewAddressPubKey(pubkey.SerializeCompressed(),
...
func (p *Pool) addUsedAddr(ns, addrmgrNs walletdb.ReadWriteBucket, seriesID uint32, branch Branch, index Index) error {
	...
	encryptedHash, err := p.manager.Encrypt(waddrmgr.CKTPublic, chainutil.Hash160(script))
	if err != nil {
...
func (p *Pool) getUsedAddr(ns, addrmgrNs walletdb.ReadBucket, seriesID uint32, branch Branch, index Index) (
	...
	addr, err := chainutil.NewAddressScriptHashFromHash(hash, mgr.ChainParams())
	if err != nil {
