...
func TestChaining(t *testing.T) {
	...
	for _, test := range tests {
		...
		if !ok {
			t.Errorf("%s: chainec signature verification failed for next keypair (chained from uncompressed pubkey).",
				test.name)
			return
		}
		...
		if !ok {
			t.Errorf("%s: chainec signature verification failed for next keypair (chained from compressed pubkey).",
				test.name)
			return
		}
	}
...
func TestWalletPubkeyChaining(t *testing.T) {
	...
	if !ok {
		t.Errorf("chainec signature verification failed; address's pubkey mismatches the privkey.")
		return
	}
	...
	ok = sig.Verify(hash, pubKey)
	if !ok {
		t.Errorf("chainec signature verification failed; next address's keypair does not match.")
		return
	}
...
func TestWatchingWalletExport(t *testing.T) {
	...
	var newAddrs []chainutil.Address
	for i := 0; i < 10; i++ {
	...
	wif, err := chainutil.NewWIF(pk, tstNetParams, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestImportPrivateKey(t *testing.T) {
	...
	// import priv key
	wif, err := chainutil.NewWIF((*chainec.PrivateKey)(pk), tstNetParams, false)
	if err != nil {
		t.Fatal(err)
	}
