...
ndrw will not drop transaction history by itself, as this is something that
...
```
$ cd $GOPATH/src/github.com/endurio/ndrw/cmd/dropwtxmgr
$ go get
```
...
```
$ dropwtxmgr
Database path: /home/username/.ndrw/mainnet/wallet.db
Drop all ndrw transaction history? [y/N] y
...
```
$ dropwtxmgr --db ~/.ndrw/testnet/wallet.db
Database path: /home/username/.ndrw/testnet/wallet.db
Drop all ndrw transaction history? [y/N] y
Dropping wtxmgr namespace
```

After dropping transaction history, ndrw may be restarted and a full rescan
